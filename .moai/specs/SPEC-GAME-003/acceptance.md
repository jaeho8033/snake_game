...
- [x] AC-THEME-1, AC-THEME-2 시나리오가 자동화 테스트(`tests/ui/Theme.test.ts`)로 통과한다.
- [x] 테마 변경이 게임 코어 로직(`src/game/`)을 변경하지 않음을 AC-THEME-2가 보증한다.
- [x] 사전 정의 테마(light, dark)가 `src/ui/themes/`에 정의된다.
- [x] 커버리지 목표 85% 이상(테마 모듈).
- [x] TRUST 5 품질 게이트 통과.
- [x] Exclusions에 명시된 항목(사용자 정의 테마, 영구 저장, 전환 애니메이션, 난이도/장애물, 코어 루프)은 구현되지 않았다.
